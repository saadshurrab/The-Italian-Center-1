...
export default function POS() {
  ...
  async function handleCheckout() {
    ...
    try {
      // 1. حفظ فاتورة البيع في جدول sales
      ...
      // 2. حفظ تفاصيل عناصر الفاتورة في جدول sale_items
      ...
      // 4. إعداد بيانات الفاتورة المكتملة
      ...
      alert('حدث خطأ أثناء إتمام عملية البيع وحفظ البيانات: ' + (err as Error).message);
    } finally {
      setProcessing(false);
    }
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-4 flex flex-col h-fit sticky top-20">
          ...
          <div className="space-y-3 border-t border-slate-200 dark:border-slate-700 pt-4">
            ...
            >
              {processing ? 'جاري المعالجة والحفظ...' : 'إتمام البيع'}
            </button>
          </div>
        </div>
      </div>
      ...
      {/* Receipt modal - الفاتورة التفصيلية */}
      <Modal open={showReceipt} onClose={() => setShowReceipt(false)} title="إيصال البيع التفصيلي" size="sm">
        {receiptData && (
          <div>
            <div className="print-area text-right dir-rtl">
              {/* ترويسة الفاتورة */}
              <div className="text-center mb-4 pb-3 border-b border-dashed border-slate-300 dark:border-slate-600">
                <h2 className="font-display font-bold text-xl text-slate-800 dark:text-white">شركة الرؤيا النقية</h2>
                <p className="text-xs text-slate-500 mt-1">فاتورة مبيعات / إيصال استلام</p>
                <div className="text-xs text-slate-400 space-y-0.5 mt-2">
                  <p>رقم العملية: <span className="font-mono font-bold text-slate-700 dark:text-slate-200">#{receiptData.saleId.slice(0, 8)}</span></p>
                  <p>التاريخ والوقت: {formatDateTime(receiptData.date)}</p>
                </div>
              </div>

              {/* بيانات العميل */}
              <div className="bg-slate-50 dark:bg-slate-700/30 p-2.5 rounded-lg mb-4 text-xs space-y-1">
                <div className="flex justify-between">
                  <span className="text-slate-400">اسم العميل:</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-200">{receiptData.customerName}</span>
                </div>
                {receiptData.customerPhone && (
                  <div className="flex justify-between">
                    <span className="text-slate-400">رقم الهاتف:</span>
                    <span className="font-mono text-slate-700 dark:text-slate-200">{receiptData.customerPhone}</span>
                  </div>
                )}
              </div>

              {/* جدول / قائمة الأغراض */}
              <div className="border-b border-dashed border-slate-300 dark:border-slate-600 pb-3 mb-3">
                  ...
                  {receiptData.items.map((item: CartItem, i: number) => (
                    <div key={i} className="text-xs space-y-0.5">
                      ...
                      <div className="flex justify-between text-[10px] text-slate-400">
                        <span>باركد: {item.barcode || 'N/A'}</span>
              ...
              <div className="space-y-1.5 text-xs pt-1">
                ...
                {receiptData.discount > 0 && (
                  <div className="flex justify-between text-error-600 dark:text-error-400 font-medium">
                    <span>الخصم</span>
                    <span>-{formatCurrency(receiptData.discount)}</span>
                  </div>
                )}

                <div className="flex justify-between font-bold text-sm text-slate-800 dark:text-white border-t border-slate-200 dark:border-slate-700 pt-2 my-1">
                  <span>الإجمالي النهائي</span>
                  <span className="text-brand-600 dark:text-brand-400">{formatCurrency(receiptData.total)}</span>
                </div>

                <div className="flex justify-between text-slate-500 pt-1">
                  <span>طريقة الدفع</span>
                  <span className="font-medium text-slate-700 dark:text-slate-200">{PAYMENT_LABELS[receiptData.paymentMethod]}</span>
                </div>
                ...
                {receiptData.change > 0 && (
                  <div className="flex justify-between font-medium text-accent-600 dark:text-accent-400">
              ...
              {/* تذييل الفاتورة */}
              <div className="text-center mt-6 pt-3 border-t border-dashed border-slate-300 dark:border-slate-600">
                <p className="text-xs font-medium text-slate-600 dark:text-slate-300">شكراً لزيارتكم!</p>
                <p className="text-[10px] text-slate-400 mt-0.5">يرجى الاحتفاظ بالظرف أو الفاتورة للمراجعة</p>
              </div>
            </div>
