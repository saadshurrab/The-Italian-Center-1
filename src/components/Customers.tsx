...
export default function Customers() {
  ...
  async function viewDetails(customer: Customer) {
    ...
    const [examRes, orderRes, saleRes] = await Promise.all([
      supabase.from('examinations').select('*').eq('customer_id', customer.id).order('exam_date', { ascending: false }),
    ...
    setViewData({ exams: examRes.data || [], orders: orderRes.data || [], sales: saleRes.data || [] });
  }
    ...
    <div>
      ...
      <div className="card p-4 mb-4">
        <div className="relative">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ابحث بالاسم أو الهاتف أو البريد..." className="input pr-10" />
        </div>
      </div>

      {/* Grid */}
      {filtered.length === 0 ? (
        <div className="card"><EmptyState message="لا يوجد عملاء" icon={<Contact className="w-10 h-10" />} /></div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((c) => (
            <div key={c.id} className="card card-hover p-4">
              ...
              {c.email && <p className="text-xs text-slate-400 flex items-center gap-1 mb-2"><Mail className="w-3 h-3" /> {c.email}</p>}
              <div className="flex items-center justify-between mb-3 text-sm">
                <span className="text-slate-500 dark:text-slate-400">الرصيد:</span>
                <span className={`font-bold ${c.balance > 0 ? 'text-error-600 dark:text-error-400' : 'text-accent-600 dark:text-accent-400'}`}>
              ...
              <div className="flex gap-1 border-t border-slate-100 dark:border-slate-700 pt-3">
                ...
                <button onClick={() => { setForm({ name: c.name, phone: c.phone || '', email: c.email || '', address: c.address || '', date_of_birth: c.date_of_birth || '', notes: c.notes || '', balance: c.balance.toString() }); setEditingId(c.id); setShowModal(true); }} className="btn-ghost text-xs">
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editingId ? 'تعديل عميل' : 'عميل جديد'} size="md">
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="input" placeholder="اسم العميل" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" value={form.balance} onChange={(e) => setForm({ ...form, balance: e.target.value })} className="input text-left" placeholder="0" />
            </div>
          </div>
          <div>
            <label className="label">العنوان</label>
            <input type="text" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className="input" placeholder="العنوان" />
          </div>
          <div>
            <label className="label">ملاحظات</label>
            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="input min-h-[60px]" placeholder="ملاحظات..." />
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="card p-4 bg-slate-50 dark:bg-slate-700/30">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><span className="text-slate-400">الرصيد: </span><span className={`font-bold ${viewCustomer.balance > 0 ? 'text-error-600 dark:text-error-400' : 'text-accent-600 dark:text-accent-400'}`}>{formatCurrency(viewCustomer.balance)}</span></div>
              </div>
