...
export default function Orders() {
  const [loading, setLoading] = setLoadingState();
  ...
  async function save() {
    ...
    const orderPayload = {
      ...
      total_amount: total,
      amount_paid: editingOrder?.amount_paid || 0,
      balance: total - (editingOrder?.amount_paid || 0),
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.customer_id} onChange={(e) => setForm({ ...form, customer_id: e.target.value, examination_id: '' })} className="input">
            ...
            <div>
              ...
              <select value={form.examination_id} onChange={(e) => setForm({ ...form, examination_id: e.target.value })} className="input" disabled={!form.customer_id}>
                <option value="">بدون وصفة</option>
                {customerExams.map((ex) => (
                  <option key={ex.id} value={ex.id}>{formatDate(ex.exam_date)} — {ex.doctor_name || 'فحص'}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="card p-3 bg-slate-50 dark:bg-slate-700/30 space-y-3">
              <div className="grid grid-cols-12 gap-2">
                <input type="text" value={newItem.item_name} onChange={(e) => setNewItem({ ...newItem, item_name: e.target.value })} placeholder="اسم العنصر" className="input col-span-4 text-sm" />
                <select value={newItem.item_type} onChange={(e) => setNewItem({ ...newItem, item_type: e.target.value })} className="input col-span-3 text-sm">
                  <option>إطار</option>
                  <option>عدسة</option>
                ...
                <input type="number" value={newItem.quantity} onChange={(e) => setNewItem({ ...newItem, quantity: parseInt(e.target.value) || 1 })} placeholder="الكمية" className="input col-span-2 text-sm text-center" />
                <input type="number" value={newItem.unit_price} onChange={(e) => setNewItem({ ...newItem, unit_price: parseFloat(e.target.value) || 0 })} placeholder="السعر" className="input col-span-2 text-sm text-left" />
                <button onClick={addItem} type="button" className="btn-primary col-span-1 px-0">
                  ...
                  {form.items.map((item, i) => (
                    <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-white dark:bg-slate-800 text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-slate-600 dark:text-slate-300">{formatCurrency(item.unit_price * item.quantity)}</span>
                        <button onClick={() => removeItem(i)} type="button" className="text-error-500"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="input min-h-[60px]" placeholder="ملاحظات الطلب..." />
          </div>
...
function setLoadingState(): [boolean, (v: boolean) => void] {
